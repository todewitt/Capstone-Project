...
def portfolio():
    ...
    for order in user.orders:
        if order.order_type == 'BUY':
            holdings[order.stock_symbol] = holdings.get(order.stock_symbol, 0) + order.quantity
...
def edit_stocks():
    ...
    if request.method == 'POST':
        ...
        try:
            new_stock = Stock(stock_symbol=stock_symbol, name=name, price_per_share=float(price_per_share), quantity=float(quantity))
